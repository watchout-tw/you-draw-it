...
var mixinGraph = {
  ...
  computed: {
    score: function () {
      if (!(this.rows && this.rows.user)) return 87;
  ...
  methods: {
    ...
    drawPath: function (el, points, title) {
      ...
      // draw circles
      var circles = el.selectAll('circle').data(points, function (d) {
        return d.x;
      });
      circles.exit().remove();
      circles.enter().append('circle').merge(circles).attr('r', this.size.r).attr('cx', function (d) {
        return self.util.axes.x.scale(d.x);
      }).attr('cy', function (d) {
        return self.util.axes.y.scale(d.y);
      }).classed('fix', function (d) {
        return d.fix;
      ...
      var labels = el.selectAll('text.data').data(endpoints, function (d) {
        return d.x;
      });
      labels.exit().remove();
      labels.enter().append('text').merge(labels).text(function (d) {
        return self.util.sequence.label.format(d.y);
      }).attr('x', function (d) {
        return self.util.axes.x.scale(d.x);
      }).attr('y', function (d) {
        return self.util.axes.y.scale(d.y) - self.size.r * 2;
      }).attr('class', 'data').classed('hide', function (d) {
        return !d.show;
      });
    ...
    init: function () {
      ...
      // x & y scale
      util.axes.x.values = this.rows.user.map(function (d) {
        return d.x;
      });
      util.axes.x.scale = d3.scalePoint().domain(util.axes.x.values).range([size.p, size.w - size.p]);
      util.axes.y.scale = d3.scaleLinear().domain([props.axes.y.min, props.axes.y.max]).range([size.h - size.p, 0 + size.p]);
      ...
      util.line = d3.line().x(function (d) {
        return util.axes.x.scale(d.x);
      }).y(function (d) {
        return util.axes.y.scale(d.y);
      });

      // x axis
      util.axes.x.axis = d3.axisBottom(util.axes.x.scale).tickSize(size.h - size.p * 2);
      util.axes.x.customize = function (g) {
        ...
        g.selectAll('.tick').each(function (d, i, nodes) {
          ...
          var text = tick.append('g').attr('transform', 'translate(' + [-util.axes.x.scale.step() / 2, size.h - size.p - size.r * 8 + 2].join(',') + ')');
      ...
      util.axes.y.axis = d3.axisRight(util.axes.y.scale).tickSize(0).tickValues(this.props.axes.y.ticks).tickFormat(function (d) {
    ...
    draw: function () {
      ...
      rectangles.enter().append('rect').merge(rectangles).attr('x', function (d) {
        return self.util.axes.x.scale(d.x);
      }).attr('y', this.util.axes.y.scale(this.props.axes.y.max)).attr('width', this.util.axes.x.scale.step()).attr('height', this.util.axes.y.scale(this.props.axes.y.min) - this.util.axes.y.scale(this.props.axes.y.max)).attr('fill', function (d) {
        return colors[d.label];
      });

      var lastPresident = 'hui';
      this.rows.user.forEach(function (row, i, rows) {
        var _row$label$split = row.label.split('-'),
            _row$label$split2 = _slicedToArray(_row$label$split, 2),
            president = _row$label$split2[0],
      ...
      // draw x axis
      this.el.x = this.el.root.append('g').attr('class', 'axis axis-x').attr('transform', 'translate(' + [0, this.size.p].join(',') + ')').call(this.util.axes.x.customize);

      // draw y axis
      this.el.y = this.el.root.append('g').attr('class', 'axis axis-y').call(this.util.axes.y.customize);
      ...
      this.el.button = d3.select(this.$el).select('.after').append('button').text('不想畫啦').on('click', function () {
        // remove animation
      ...
      this.el.container.select('.you-draw').style('top', this.util.axes.y.scale(lastOrig.y) * zoom - 54 + 'px').style('left', this.util.axes.x.scale(lastOrig.x) * zoom + 'px').style('transform', 'scale(' + zoom + ')').style('transform-origin', 'center left');
    }
  }
};
